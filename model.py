...
src_vocab_size = 85483   # 注释字典长度
# tgt_vocab_size = 2657      # api 字典长度
tgt_vocab_size = 20317      # api 字典长度
...
#     # def __init__(self, vocab_size, embedding_dim, weights=None, is_pretrain=False):
...
#         self.word_query = nn.Parameter(torch.Tensor(d_size, 1), requires_grad=True)     # 定义一个学习矩阵,d size是词嵌入维度，你的好像是100
#         self.word_fc = nn.Linear(d_size, d_size)
#         nn.init.xavier_normal_(self.word_query, gain=1)   # 初始化学习矩阵
#
#     def forward(self, x):
#         x = x.to(device)
#         sentence_num = x.size(1)    # sentence_num 一条注释最多有多少单词 20
#         # sentence_num = 1
#         sentence_len = x.size(2)    # 3
#         x = x.view(-1, sentence_len, self.embedding_dim)
#
#         # embed_x = self.word_embed(x)     # 假设这就是你那个3*d size的矩阵，shape为(batch size,3,d size)
#         embed_x = x     # 假设这就是你那个3*d size的矩阵，shape为(batch size,3,d size)
#         # embed_x = embed_x.to(device)
#         word_attention = torch.tanh(self.word_fc(embed_x).to(device))  # 激活函数
#         weights = torch.matmul(word_attention, self.word_query)     # 计算权重矩阵
#         weights = F.softmax(weights, dim=1)
#         # x = x.unsqueeze(2)
#         weights = weights / (torch.sum(weights, dim=1).unsqueeze(1) + 1e-4)   # 为了精确加的一步
#         context_vector = torch.sum(weights * embed_x, dim=1).view(-1, sentence_num, embed_x.size(2))    # 最终得到的上下文向量
#         # context_vector = torch.sum(weights * embed_x, dim=1).view(sentence_num, embed_x.size(1))    # 最终得到的上下文向量
#         return context_vector


class PositionalEncoding(nn.Module):
    def __init__(self, d_model, dropout=0.1, max_len=20317):
...
# def get_enc_attn_pad_mask(seq_q, seq_k):                                # seq_q: [batch_size, seq_len] ,seq_k: [batch_size, seq_len]
#     batch_size, len_q, d = seq_q.size()
#     batch_size, len_k, _ = seq_k.size()
#     pad_attn_mask = seq_k.data[:, :, 1].eq(0).unsqueeze(1)                   # 判断 输入那些含有P(=0),用1标记 ,[batch_size, 1, len_k]
#     return pad_attn_mask.expand(batch_size, len_q, len_k)           # 扩展成多维度
#
#
# def get_enc_dec_attn_pad_mask(seq_q, seq_k):                                # seq_q: [batch_size, seq_len] ,seq_k: [batch_size, seq_len]
...
class Encoder(nn.Module):
    def __init__(self):
        ...
        # self.src_emb = nn.Embedding(d_model, 1)                     # 把字向量转换数字
    ...
    def forward(self, enc_inputs):                                               # enc_inputs: [batch_size, src_len]
        ...
        # enc_outputs = self.src_emb(enc_outputs)
        enc_self_attn_mask = get_attn_pad_mask(enc_inputs, enc_inputs)           # enc_self_attn_mask: [batch_size, src_len, src_len]
        # enc_self_attn_mask = get_enc_attn_pad_mask(enc_inputs, enc_inputs)           # enc_self_attn_mask: [batch_size, src_len, src_len]
        enc_self_attns = []
...
class Decoder(nn.Module):
    def __init__(self):
        ...
        # self.tran_matrix = torch.rand(token_vec_len, d_model).double().to(device)
    ...
    def forward(self, dec_inputs, enc_inputs, enc_outputs):                         # dec_inputs: [batch_size, tgt_len]
        ...
        # dec_enc_attn_mask = get_enc_dec_attn_pad_mask(dec_inputs, enc_inputs)               # [batc_size, tgt_len, src_len]
...
class Transformer(nn.Module):
    def __init__(self):
        ...
        self.projection = nn.Linear(d_model, tgt_vocab_size, bias=False).to(device)     # 100 26317
